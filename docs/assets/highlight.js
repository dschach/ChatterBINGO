	...
	function Q(e) {
		...
		return (
			...
			(function t(i, s) {
				...
				[D, B, W, z].forEach((e) => e(i, s)), e.compilerExtensions.forEach((e) => e(i, s)), (i.__beforeBegin = null), [I, L, $].forEach((e) => e(i, s)), (i.isCompiled = !0);
				let l = null;
				return (
					'object' == typeof i.keywords && i.keywords.$pattern && ((i.keywords = Object.assign({}, i.keywords)), (l = i.keywords.$pattern), delete i.keywords.$pattern),
					(l = l || /\w+/),
			...
			try {
				...
				if (n.message && n.message.includes('Illegal'))
					return {
						...
						illegal: !0,
						relevance: 0,
						_illegalBy: { message: n.message, index: C, context: t.slice(C - 100, C + 100), mode: n.mode, resultSoFar: k },
						_emitter: M
					};
				if (s)
					return {
	...
	var Ue = Object.freeze({
		...
		grmr_kotlin: (e) => {
			...
			const s = {
				...
				o = { className: 'meta', begin: '@' + e.UNDERSCORE_IDENT_RE, contains: [{ begin: /\(/, end: /\)/, contains: [e.inherit(r, { className: 'string' })] }] },
				l = ge,
				...
				contains: [
					{
						...
						relevance: 10,
						contains: [i, o, s, r, { begin: /\[/, end: /\]/, contains: [{ className: 'meta', begin: /<![a-z]/, end: />/, contains: [i, r, o, s] }] }]
					},
					e.COMMENT(/<!--/, /-->/, {
						relevance: 10
					}),
		...
		grmr_scss: (e) => {
			...
			return {
				...
				contains: [
					...
					{ begin: /:/, end: /[;}{]/, contains: [n.BLOCK_COMMENT, r, n.HEXCOLOR, n.CSS_NUMBER_MODE, e.QUOTE_STRING_MODE, e.APOS_STRING_MODE, n.IMPORTANT] },
		...
		return (e) => {
			const s = e.regex,
				...
				o = e.COMMENT('/\\*', '\\*/', {
					...
					contains: [
						...
						{ begin: '`', end: '`', excludeBegin: !0, excludeEnd: !0, scope: 'code', contains: [e.BACKSLASH_ESCAPE], relevance: 0 },
						e.APOS_STRING_MODE,
						{
							match: [/(?<=@param)/, /\s+/, /\w+/],
							scope: { 3: 'variable' }
				...
				i = { relevance: 10, scope: { 1: 'meta' }, match: ['@' + t, /(?=(\(|\b|\s))/] },
				l = [
					{
				...
				b = [{ match: [s.concat(/\b/, t, /\b/), />/], scope: { 1: 'type' }, relevance: 10 }],
				p = [
					{
				...
				A = {
					variants: [
						{ begin: [/\./, s.concat('(?:' + t + ')'), /(?=\s*\(\))/], scope: { 2: 'title.function.invoke' } },
						{
							begin: [/\./, s.concat('(?:' + t + ')'), /(?=\s*\([^\)])/],
							scope: {
				...
				u = {
					...
					contains: [
						...
						{ match: /(\bIN\b|<|<=|>|>=|\bNOT\s+IN\b|=|!\s*=|:{1})/, scope: 'literal', relevance: 0 },
			...
			return {
				...
				contains: [
					i,
					[
						{ match: [/\b(?<=enum|\bnew)/, /\s+/, t, /\s*(?=[{()])/], scope: { 3: 'type' }, contains: [r, o, E] },
						{ match: [/(?<=\bclass\b)/, /\s+/, t], scope: { 3: 'title.class' } },
						{
							match: [/(?<=public)/, /\s+/, t, /(?=\s*\()/],
							scope: { 3: 'constructor' }
						},
					...
					[
						...
						{ match: [t, /\s+/, t, /\s+/, ';'], scope: { 1: 'type', 3: 'variable' }, relevance: 0 },
						{ match: [/\s+/, t, /\s+/, /=/], scope: { 2: 'variable', 4: 'operator' }, relevance: 0 },
						{ match: [/(?<=\w+\s+=\s+\()/, t, /(?=\))/], scope: { 2: 'type' }, relevance: 0 }
					],
					{ relevance: 10, match: [/\b(?:with|without|inherited)\s+sharing/], scope: { 1: 'keyword' } },
					p,
					b,
					o,
					r,
